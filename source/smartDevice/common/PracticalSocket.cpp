...
int CommunicatingSocket::recv_withTimeOut(void *buffer, int bufferLen, int timeOut) 
    throw(SocketException) {
/*
    fd_set s;
    FD_ZERO(&s);
    ...
    int retval = select(sockDesc + 1, &s, &s, &s, &timeout);
    if(retval == -1)  {
      return -1;
    }
    if(retval > 0)  {
...
*/
    ...
    fd.fd = sockDesc;
    fd.events = POLLIN;
    res = poll(&fd, 1, timeOut); // 1000 ms timeout
